...
use ftd2xx_rs::{
    FtError, classic, ee_ua_size,
    types::{FtHandle, MyProgramData},
};
use rand::{Rng, RngExt};
use regex::bytes;
...
fn test_eeprom_by_changing_serial_number() -> Result<(), FtError> {
    ...
    assert!(
        &new_eeprom_data.serial_number[..new_serial_number.len()] == new_serial_number.as_bytes()
    );
...
fn test_eeprom_by_changing_configuration() -> Result<(), FtError> {
    ...
    let new_drive_current = if old_drive_current == 16 {
        4
    } else {
        old_drive_current + 4
    };
